...
for k in range(h):
    ...
    target[k] = b
"""
target = np.load('n3unique_target_pol.npy')
design = np.load('n3Jpol_design.npy')
#target = target[:25000]
#design = design[:25000]
...
# add layers
model.add(Dense(2**(n),input_dim=n_j-1, activation='relu'))
...
plt.xlabel('Predicted Eigenvalue')
plt.ylabel('True Eigenvalue')
plt.title("Unique Eigenvalues: n=5")
plt.plot([-2,2], [-2,2], linestyle='dashed', color='k')
plt.grid()
#plt.savefig('5N_un.eps', format='eps', dpi=1200)
plt.show()
