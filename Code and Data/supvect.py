...
for C in Cs:
    ...
    y_train_pred = y_mu + model.predict(X_train_std)*y_std
    y_val_pred = y_mu + model.predict(X_val_std)*y_std

    train_mse.append(mean_squared_error(y_true=y_train, y_pred=y_train_pred))
    val_mse.append(mean_squared_error(y_true=y_val, y_pred=y_val_pred))
...
# We want the model with the lowest validation mse
